...
class ImgWelcome(commands.Cog):
	...
	async def _create_welcome(J,member,test_member_number:int=_B):
		w='of ';v='Welcome';u='L';U=test_member_number;D=member;A=await J.config.guild(D.guild).all();h=A[_L];i=A[_M];j=A[_J];k=A[_K];K=A[_E];l=A[_F];m=A[_G];n=A[_H];o=A[_I];V=ImageFont.truetype(h,i);L=ImageFont.truetype(j,k);W=ImageFont.truetype(K,l);X=ImageFont.truetype(K,m);Y=ImageFont.truetype(K,n);Z=ImageFont.truetype(K,o);a=Image.open(A[_O]).convert(_D);p=Image.open(J.imgpath+'/noimage.png');E=Image.new(_D,(500,500));E=ImageOps.fit(a,(500,500),centering=(0.5,0.5));E.paste(a);E=E.resize((500,500),Image.NEAREST);M=Image.new(u,(512,512),0);q=ImageDraw.Draw(M);q.ellipse(((0,0),(512,512)),fill=255);P=tuple(A[_Q]);M=M.resize(P,Image.ANTIALIAS)
	...
	async def imgwelcome_upload(self,ctx,default=_B):
		'Upload a background through Discord. 500px x 500px.\n        This must be an image file and not a url.';B=self;A=ctx;await A.send('Please send the file to use as a background. File must be 500px x 500px.')
		...
		if C:
			...
			if C:
				if D.size==(500,500):await B.config.guild(A.guild).BACKGROUND.set(f"{B.datapath}/guilds/{A.guild.id}/serverbg.png")
				else:return await A.send('Image needs to be 500x500.')
